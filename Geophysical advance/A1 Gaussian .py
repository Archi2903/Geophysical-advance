...
def gaussian_elimination(A, B): # def - ключевое слово, которое используется для обьявления функции # gaussian_elimination - название функции # A, B - аргументы функции
    ...
    for i in range(n): # range - это функция, которая возвращает последовательность чисел от 0 до i-1
        ...
        max_row = np.argmax(np.abs(A[i:n, i])) + i  # Индекс максимального элемента в столбце , не работает когда все элементы в столбце или строке равны 0
        if A[max_row, i] == 0:
            raise ValueError("Матрица несовместна или плохо обусловлена.")
        
        # Обмен строками (swapping two equations)
        A[[i, max_row]] = A[[max_row, i]] # A[[i, max_row]] - это обращение к строкам матрицы A, где i=0 (первая строка A[0]1,2,3), а max_row - это индекс строки с максимальным элементом у нас это A[2]1,3,4
